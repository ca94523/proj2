...
filepath = paste0(getwd(), "/app/services")
...
pdscore_banking <- function(json_input,con,cfmast_data,ddmast_data,ddhist_data,listcif){
  options(scipen = 999)
  ...
  if(length(df_year_in_bank) == 0 ){
    ...
    if(length(del_cif) == 0 ){
      ...
      # PENGECEKAN CIF DI DDHIST
      if(length(ddhist_ag) == 0 ){
        ddhist_ag5 <- data.frame(cifno = demog_data[['CIF']] , ratio_trlocadd_c_avg6_12 = NA , ratio_trlocadd_num_c_avg3_12 = NA )
      }else {
    ...
    ####  PENGECEKAN CIF DI CDMAST.
    ###### Jika CIF tidak ditemukan, cdhist isinya NA
    
    
    if(length(del_cif1_x) == 0 ){
      cdhist <- data.frame(cifno = demog_data[['CIF']] , cd_avg_tx_amt_d_12 = NA )
    }else {
...
pdscore_digital <- function(json_input,con,ddmast_data,ddhist_data,listcif){     
  ...
  if (length(ebwmemberaccountchannel_tr_th) == 0){
  ...
  if(nrow(ddhist_ag_digi) == 0 && length(ebwmemberaccountchannel_tr_th) == 0){
